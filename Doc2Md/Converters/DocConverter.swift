...
struct DocConverter {
    func convert(url: URL) throws -> String {
        ...
        return convertToMarkdown(attrString)
    }

    private func convertToMarkdown(_ attrString: NSAttributedString) -> String {
        ...
        text.enumerateSubstrings(in: text.startIndex..<text.endIndex, options: .byParagraphs) { substring, range, _, _ in
            ...
            let (prefix, strippedText) = self.detectListAndStrip(paraText)
            let heading = self.detectHeading(attrString, range: nsRange, bodyFontSize: bodyFontSize)

            let formatted = self.formatInlineAttributes(attrString, range: nsRange, paragraphText: paraText)

            var line = heading + prefix
            if !prefix.isEmpty && !strippedText.isEmpty {
                let formattedStripped = self.formatInlineAttributes(
                    attrString,
                    range: nsRange,
    ...
    private func detectHeading(_ attrString: NSAttributedString, range: NSRange, bodyFontSize: CGFloat) -> String {
    ...
    private func detectListAndStrip(_ text: String) -> (String, String) {
    ...
    private func formatInlineAttributes(_ attrString: NSAttributedString, range: NSRange, paragraphText: String, stripPrefix: Int = 0) -> String {
        var result = ""
    ...
    private func cleanupMarkdown(_ md: String) -> String {
        var result = md
        // Merge adjacent bold+italic markers from consecutive runs: ***a******b*** → ***ab***
        result = result.replacingOccurrences(of: "******", with: "")
        // Merge adjacent bold markers from consecutive runs: **a****b** → **ab**
        result = result.replacingOccurrences(of: "****", with: "")
